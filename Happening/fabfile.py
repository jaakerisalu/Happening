...
def live():
    defaults()
    env.hosts = ['tg-dev']
    env.tag = 'Happening-live'
    env.service_name = "gunicorn-Happening"

    env.code_dir = '/srv/Happening'
...
def setup_server():
    ...
    # Create database
    sudo('echo "CREATE DATABASE Happening; '
         '      CREATE USER Happening WITH password \'%s\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE Happening to Happening;" '
    ...
    put(local_path=StringIO(local_settings), remote_path=env.code_dir + '/Happening/settings/local.py', use_sudo=True)

    # Create necessary dirs, with correct permissions
    mkdir_wwwdata('/var/log/Happening/')
    with cd(env.code_dir + '/Happening'), prefix('. ../venv/bin/activate'):
    ...
    sudo('chown -R www-data:www-data /var/log/Happening/')

    # Copy nginx and gunicorn confs
    with cd(env.code_dir):
        sudo('cp deploy/%s /etc/nginx/sites-enabled/Happening' % env.nginx_conf)
        sudo('cp deploy/gunicorn.conf /etc/init/gunicorn-Happening.conf')
...
def collectstatic():
    with cd(env.code_dir + '/Happening'):
